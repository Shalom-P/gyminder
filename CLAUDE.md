...
`src/styles.css` is the single source of design truth — the **"Volt"** system (a bolder reinvention of the earlier "Aura"): an electric-violet accent, borderless/translucent surfaces, light+dark via `:root[data-theme]`. It is **token-driven** — colors, type scale, radii and shadows live on `:root`, so most restyling is done by editing tokens, not individual rules. Typography is **bundled and self-hosted** (fully offline, no CDN) via Fontsource, imported in `src/main.tsx`: **Space Grotesk** (`--font-display`) for screen titles, the brand and every hero numeral — its mono-heritage digits stay tabular so the rest timer / weights / reps never jitter — over **Inter** (`--font-ui`) for all UI/body text. `NavStack` and the original visual language were ported from standalone HTML design prototypes ("Claude Design"). Prefer existing tokens/classes over new ad-hoc styles.
